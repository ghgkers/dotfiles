...
# Оверлей для изменения цветов DWM на фиолетовые
...
# Команда сессии: Установка разрешения 165Hz и запуск скрипта статус-бара в цикле
...
while true; do
  ...
  # На ноутбуках ASUS GPU может меняться, эта команда пытается найти Nvidia или AMD
  gpu=$(${pkgs.procps}/bin/top -bn1 | grep -i 'gpu' | head -n1 | ${pkgs.gawk}/bin/awk '{print $2"%"}')
  if [ -z "$gpu" ]; then gpu="?"; fi
  ram=$(${pkgs.procps}/bin/free -h | grep "Mem" | ${pkgs.gawk}/bin/awk '{print $3"/"$2}')
  dte=$(date +"%H:%M %d/%m")
  ${pkgs.xorg.xsetroot}/bin/xsetroot -name "Vol:$vol | Bat:$bat | CPU:$cpu | GPU:$gpu | RAM:$ram | $dte"
  sleep 2
done &
'';
}
