...
packages=with pkgs;[ghostty kitty hyprland waybar fastfetch vesktop ayugram-desktop librewolf pavucontrol wofi yazi micro nitch git htop brightnessctl flameshot xclip wireplumber acpi lm_sensors gawk xorg.xsetroot procps xfce.thunar];};
programs={steam.enable=true;gamemode.enable=true;hyprland.enable=true;bash.shellAliases={
  dotsync="cd ~/dotfiles&&sudo cp /etc/nixos/*.nix .&&cp -r ~/.config/hypr ~/.config/waybar .&&git add .&&git commit -m \"upd:$(date +%F)\"&&git push origin main&&cd -";
